...
const SingleBlog = ({ blog }: { blog: any }) => {
  ...
  return (
    <div className='wow fadeInUp group relative isolate'>
      <div className='relative mb-6 h-[222px] w-full overflow-hidden rounded-xl bg-gradient-to-br from-purple-500/30 via-slate-800 to-slate-900'>
        {mainImage ? (
          <img
            src={mainImage}
            alt={title}
            className='h-full w-full scale-100 object-cover duration-500 ease-linear group-hover:scale-125'
          />
        ) : (
          <div className='flex h-full w-full items-center justify-center text-sm font-medium uppercase tracking-[0.2em] text-white/70'>
            Article
          </div>
        )}
      </div>
